...
class NodeAnswer < Knowledge_tree
  ...
  def inspect
    puts "#{NodeAnswer.new(self.value)}"
  end
  ...
  def learn_new_thang
    ...
    $new_node = {:thing => new_answer, :question => new_question, :child => distinction}
  end
...
class NodeQuestion < Knowledge_tree
  ...
  def inspect
    print "#{NodeQuestion.new(self.value)}" + "\t\t" + "#{self.yes.inspect}" + "\n\t\t" + "#{self.no.inspect}"
  end
...
# this function however, is, dare I say... EVEN SEXIER?!?
  ...
  begin
    ...
    case opt
      when 1 then
        ...
        # this it the only part of my program that feels 'uncomfortable'. It breaks with the elegance the rest of the
        # control flow exhibits. We have to implement this out of recursion however, if we are going to modify the tree
        # at the root level. It's a bit 'hacky', but does the job.
        if $new_node
          tree = $new_node[:child] =~ /Y|y/ ?
              NodeQuestion.new($new_node[:question], NodeAnswer.new($new_node[:thing]), tree) :
              NodeQuestion.new($new_node[:question], tree, NodeQuestion.new($new_node[:thing]))
        end
      when 2 then
        puts ''
